...
<div v-if = "arr.length > 0">
    <h2>总价为：￥{{allMoney}}  当前选中的书籍为：{{currentName}}  您上次删除的书籍为：{{delName}}</h2>
        <table border="1">
            ...
            <BookDetails v-for = "(item,index) of arr" :key="item.bookName" :bookName="item.bookName" :price="item.price" :counter="item.counter" :index = "index" @move = "change" @money = "moneys" :class = "{active:index == currentIndex}" @click = "select(index)" @moremsg = "moreBookmsg"></BookDetails>
        </table>
